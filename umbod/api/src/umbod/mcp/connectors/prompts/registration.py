...
from umbod.mcp.connectors.prompts.registry import RuntimeConnectorPromptRegistry


class ConnectorPromptRegistrationOptions(Model):
    ...
    connector_prompt_mappings: SkipValidation[Sequence[ConnectorPromptMapping] | None]


async def register_connector_prompts(
    mcp: FastMCP,
    options: ConnectorPromptRegistrationOptions,
) -> None:
    if not _has_required_registration_options(options):
        return
    registry = RuntimeConnectorPromptRegistry(
        mcp,
        ...
        connector_prompt_mappings=cast(
            Sequence[ConnectorPromptMapping], options.connector_prompt_mappings
        ),
    )
    setattr(mcp, "connector_prompt_registry", registry)
...
def _has_required_registration_options(options: ConnectorPromptRegistrationOptions) -> bool:
    return bool(
        ...
        and options.connector_prompt_mappings
    )
